...
dma$DisplayDMA<-dma$DMA_COL
dma$DMA_COL<-gsub("(?<=\\S)[aeiou]", "", dma$DMA_COL, perl = TRUE, ignore.case = TRUE)
dma<-dma %>%
  select(DMA_COL,DisplayDMA,`Nielsen Market - Abbreviated`,`Nielsen Market - Full Name`,`IRI Market - Abbreviated`,TVHHPercent)
...
##CREATE REACTIVE VALUE FOR INPUT DATA
  myData<-reactive({
    if (is.null(input$file1)) {
      ...
      csv<-read_csv(input$file1$datapath)
    }
  })
